...
class PostProvider extends React.Component {
    ...
    componentDidMount() {
        const {context, page, perPage, search, type, subtype, locale, store = "results", apiBaseUrl} = this.props
        if (search && search !== "") {
            this.props.onSearch({context, page, perPage, search, type, subtype, locale, store, apiBaseUrl})
        }
    }
